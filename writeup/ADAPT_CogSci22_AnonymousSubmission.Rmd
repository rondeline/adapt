...
quiet ~ activity + (activity | subject_id), where quiet refers to the number of times the quiet room was selected as the response, and both the activity in which participants were exposed and participants' individual responses were entered as random effects in the model. Figure 5 depicts adult participants' preferences for quieter environments based on the chosen activity. Activities had a marginal effect on room selection (intercept:  $\beta$ = `r round(summary_2a[1,][["mean"]],2)`, Crl = [`r round(summary_2a[1,][["2.5%"]],2)` - `r round(summary_2a[1,][["97.5%"]],2)`])
We found that all but two activities were either preferred in the quiet or loud room- eating (intercept : $\beta$ = 0.1, Crl = [-0.5-0.8]) and talking (intercept : $\beta$ = 0.5, Crl = [-0.06-1.2]), which were almost equally preferred in both rooms. Reading was most preferred in the quiet room while dancing was most preferred in the loud room.
...
```{r, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=2, fig.height=2, set.cap.width=T, num.cols.cap=1, fig.cap = "Experiment 2b. Proportion of participants selecting the quieter room by activity."}
...
glmer_2b <- brm(formula = quiet ~ activity + 
                  (activity | subject_id),
                family = "bernoulli",
